...
def test_day2_delete_creates_tombstone_and_disappears_from_retrieval(tmp_path: Path, monkeypatch):
    _setup(tmp_path)
    ...
    p = tmp_path / "brain" / "domains" / "sg_acabados" / "to_delete.md"
    p.write_text("Factura y cobranza especial\n", encoding="utf-8")
    run_day2_inventory_sync(tmp_path)

    p.unlink()
    out = run_day2_inventory_sync(tmp_path)

    assert out["summary"]["deleted"] >= 1
    tomb = tmp_path / "state" / "doc_tombstones.ndjson"
    assert tomb.is_file()

    r = retrieve(tmp_path, query="cobranza especial", principal_ctx={"user_id": "u1"})
    paths = {x["path"] for x in r["final_topk"]}
    assert all("to_delete.md" not in p for p in paths)
